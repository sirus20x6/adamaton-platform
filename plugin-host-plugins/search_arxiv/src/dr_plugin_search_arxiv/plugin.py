"""arXiv search plugin — gRPC façade around the legacy adapter."""
...
# Legacy SourceKind is a str-enum; the SDK's is an IntEnum. Map by name.
...
class Plugin:
    ...
    async def fetch(self, result: SearchResult) -> FetchedDoc:
        # The legacy adapter raises NotImplementedError for fetch — preserve
        # that contract; the host surfaces it as an UNIMPLEMENTED-ish error.
        ...
        doc = await self._adapter.fetch(legacy)  # raises
        return FetchedDoc(
            adapter=doc.adapter,
            ...
            body=doc.body if isinstance(doc.body, bytes) else doc.body.encode(),
