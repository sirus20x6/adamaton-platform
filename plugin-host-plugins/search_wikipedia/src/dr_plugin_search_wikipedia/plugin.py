"""SDK-decorated Plugin class for the Wikipedia adapter."""
...
def _from_sdk_result(r: SdkSearchResult) -> _adapter.SearchResult:
    ...
    return _adapter.SearchResult(
        ...
        source_kind=inv.get(r.source_kind, _adapter.SourceKind.WIKI),
    )
...
class Plugin:
    def __init__(self) -> None:
        self._adapter: _adapter.WikipediaAdapter | None = None

    def _get(self) -> _adapter.WikipediaAdapter:
        if self._adapter is None:
            self._adapter = _adapter.WikipediaAdapter()
