...
from dr_plugin_search_pypi.plugin import Plugin
...
async def test_query_returns_search_page() -> None:
    plugin = Plugin()
    try:
        page = await plugin.query("fastapi", 3, None, None)
    except Exception as e:
        if os.getenv("PLUGIN_LIVE_TEST"):
            raise
        pytest.skip(f"pypi upstream flaky: {type(e).__name__}: {e}")
