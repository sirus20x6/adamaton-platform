"""Smoke test for search_hackernews."""
...
from dr_plugin_search_hackernews.plugin import Plugin
...
async def test_query_returns_search_page() -> None:
    plugin = Plugin()
    try:
        page = await plugin.query("rust async", 3, None, None)
    except Exception as e:
        if os.getenv("PLUGIN_LIVE_TEST"):
            raise
        pytest.skip(f"hackernews upstream flaky: {type(e).__name__}: {e}")
    ...
    if page.results:
        ...
        assert first.adapter == "hackernews"
