"""Sanity test for the Stack Exchange search plugin."""
...
from dr_plugin_search_stackexchange.plugin import Plugin
...
@pytest.mark.skipif(
    ...
    reason="set PLUGIN_LIVE_TEST=1 to hit api.stackexchange.com",
)
async def test_query_live() -> None:
    p = Plugin()
    page = await p.query("python asyncio", 5, None, None)
