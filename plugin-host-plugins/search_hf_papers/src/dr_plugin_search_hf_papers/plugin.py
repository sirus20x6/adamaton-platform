"""SDK-decorated Plugin class. Wraps HFPapersAdapter and converts the
legacy SearchPage/SearchResult/FetchedDoc to the SDK equivalents."""
...
# Legacy string-enum -> SDK IntEnum. Same set of values; just a different
# representation.
...
def _to_sdk_result(r: _adapter.SearchResult) -> SdkSearchResult:
    return SdkSearchResult(
        ...
        # SDK uses datetime|None directly; pass through.
...
def _from_sdk_result(r: SdkSearchResult) -> _adapter.SearchResult:
    # Reverse map; the adapter only needs the bits it touches in fetch().
    ...
    return _adapter.SearchResult(
        ...
        source_kind=inv.get(r.source_kind, _adapter.SourceKind.WEB),
    )


def _to_sdk_doc(d: _adapter.FetchedDoc) -> SdkFetchedDoc:
    # SDK body is bytes; coerce str -> utf-8.
    body: bytes
    if isinstance(d.body, str):
        ...
        body = bytes(d.body)
    md = dict(d.metadata)
    md["source_tier"] = d.source_tier  # preserve under metadata for clients
...
class Plugin:
    # Lazy adapter init so __init__ is cheap; the SDK instantiates Plugin
    # inside the running event loop.
    def __init__(self) -> None:
        self._adapter: _adapter.HFPapersAdapter | None = None

    def _get(self) -> _adapter.HFPapersAdapter:
        if self._adapter is None:
            self._adapter = _adapter.HFPapersAdapter()
