"""OpenAlex search plugin — gRPC façade around the legacy adapter."""
...
class Plugin:
    ...
    async def fetch(self, result: SearchResult) -> FetchedDoc:
        # The OpenAlex fetch path needs raw[primary_location.pdf_url] — preserve
        # whatever the caller round-tripped through the wire.
