"""SDK-decorated Plugin class. Wraps GithubAdapter and converts the
...
def _from_sdk_result(r: SdkSearchResult) -> _adapter.SearchResult:
    ...
    return _adapter.SearchResult(
        ...
        source_kind=inv.get(r.source_kind, _adapter.SourceKind.REPO),
    )
...
class Plugin:
    def __init__(self) -> None:
        self._adapter: _adapter.GithubAdapter | None = None

    def _get(self) -> _adapter.GithubAdapter:
        if self._adapter is None:
            self._adapter = _adapter.GithubAdapter()
